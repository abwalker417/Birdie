...
pct create "$CTID" "$DEFAULT_OS_TEMPLATE" \
  --hostname "$HOSTNAME" \
  --password "$PASSWORD" \
  --cores "$CORES" \
  --memory "$MEMORY" \
  ...
  --net0 "$NET0" \
  --features nesting=1 \
  --unprivileged 1 || fail "Container creation failed"

pct start "$CTID" || fail "Container start failed"
sleep 8
pct exec "$CTID" -- bash -lc 'apt-get update -qq && apt-get install -y curl git sudo' || fail "Bootstrap package install failed"
pct exec "$CTID" -- bash -lc "curl -fsSL ${REPO_RAW_BASE}/install.sh -o /root/install.sh && chmod +x /root/install.sh && /root/install.sh" || fail "Birdie install failed"

IP=$(pct exec "$CTID" -- hostname -I | awk '{print $1}' | xargs) || true
msg "Birdie install finished"
